...
for file_idx, file_path in enumerate(embedding_files, 1):
    ...
    if embeddings.shape[0] != coordinates.shape[0]:
        raise ValueError(
            f"The number of embeddings does not match the number of coordinates in the file {file_path}"
        )
...
print(
    f"Mean of embeddings (per dimension): {mean_embedding_overall[:5]}..."
)  # Show only the first 5 for brevity
print(
    f"Standard deviation of embeddings (per dimension): {std_embedding_overall[:5]}...\n"
)
...
print(
    f"Exploratory analysis of embeddings completed successfully. All visualizations have been saved in the directory '{output_dir}'."
)
