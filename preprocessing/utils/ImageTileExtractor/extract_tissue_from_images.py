...
def threshold(image, size, mpp, base_mpp, mult=1):
    ...
    w = int(np.round(image.shape[0]*1./(size*mpp/base_mpp))) * mult
    h = int(np.round(image.shape[1]*1./(size*mpp/base_mpp))) * mult
    # print(w, h)
    # print(np.ceil(image.shape[0]/w), np.ceil(image.shape[1]/h))
    thumbnail = image[::int(np.ceil(image.shape[0]/w)), ::int(np.ceil(image.shape[1]/h)), :] #.get_thumbnail((w,h))
...
def add2offset(img, image, patch_size, mpp, maxmpp):
    size_x = img.shape[1]
    size_y = img.shape[0]
    offset_x = np.floor((image.shape[0]*1./(patch_size*mpp/maxmpp)-size_x)*(patch_size*mpp/maxmpp))
    offset_y = np.floor((image.shape[1]*1./(patch_size*mpp/maxmpp)-size_y)*(patch_size*mpp/maxmpp))
...
def plot_extraction(image, patch_size=224, mpp=0.5, min_cc_size=10, max_ratio_size=10, dilate=False, erode=False, prune=False, overlap=1, maxn=None, bmp=None, oversample=False, mult=1, base_mpp=None, save=''):
    ...
    grid = make_sample_grid(image, patch_size, mpp=mpp, min_cc_size=min_cc_size, max_ratio_size=max_ratio_size, dilate=dilate, erode=erode, prune=prune, overlap=overlap, maxn=maxn, bmp=bmp, oversample=oversample, mult=mult, base_mpp=base_mpp)
