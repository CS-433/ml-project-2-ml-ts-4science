...
def extract_and_rescale_tile(slide: openslide.OpenSlide, x: int, y:int, base_mpp: float, target_mpp: float, patch_size:int) -> np.ndarray:
...
def process_slide(slide_path: str, tile_coordinates: List[Tuple[int, int]], base_mpp: float, target_mpp: float, patch_size: int, output_file: str) -> None:
...
def process_slides_in_batches(slides: List[str], metadata_paths: List[str], base_mpp: float, target_mpp: float, slides_per_file: int, output_dir: str) -> None:
    ...
    for file_index in range(num_files):
        ...
        for slide_path, metadata_path in zip(slides[start_index:end_index], metadata_paths[start_index:end_index]):
            patch_size, tile_coordinates = read_metadata(metadata_path)
            process_slide(slide_path, tile_coordinates, base_mpp, target_mpp, patch_size, output_file)
