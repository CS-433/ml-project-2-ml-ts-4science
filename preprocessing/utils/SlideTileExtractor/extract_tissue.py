...
def slide_base_mpp(slide, slide_id):
    try:
        ...
        try:
            ...
            print(
                f"Slide {slide_id}: No mpp property, skip slide, other properties: {slide.properties}"
            )
            base_mpp = None
    return base_mpp
...
def threshold_stride(slide, stride):
    W = slide.dimensions[0]
    H = slide.dimensions[1]
    w = int(np.ceil(W * 1.0 / stride))
    h = int(np.ceil(H * 1.0 / stride))
    thumbnail = slide.get_thumbnail((w, h))
    thumbnail = thumbnail.resize((w, h))
...
def plot_extraction(
    ...
    thumb = slide.get_thumbnail(
        (
            np.round(slide.dimensions[0] / downsample),
            np.round(slide.dimensions[1] / downsample),
        )
    )
