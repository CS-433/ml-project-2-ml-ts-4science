...
def slide_base_mpp(slide, slide_id):
    try:
        base_mpp = float(slide.properties[openslide.PROPERTY_NAME_MPP_X])
    except KeyError:
        try:
            base_mpp = 10000/float(slide.properties['tiff.XResolution'])
        except KeyError:
            print(f"Slide {slide_id}: No mpp property, skip slide, other properties: {slide.properties}")
            base_mpp=None
    return base_mpp
...
def make_sample_grid(slide, patch_size=224, mpp=0.5, min_cc_size=10, max_ratio_size=10, dilate=False, erode=False, prune=False, overlap=1, maxn=None, bmp=None, oversample=False, mult=1, centerpixel=False, base_mpp=None):
    '''
